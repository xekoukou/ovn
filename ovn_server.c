			    ...
			    server_requests_t * server_requests)
{
	...
	case ST_NEW_NODE_SAVED:;
		...
		process_response(response,
				 (server_request_t *) server_request, server_requests);
		     ...
		     zmsg_t * msg)
{
	...
	if (strcmp(type, "retrieveRequest") == 0) {
		...
		*server_request = calloc(sizeof(new_node_request_t), 1);
		...
		process_newNodeRequest(ovndb, (new_node_request_t *) *server_request, server_requests);
		      ...
		      server_requests_t * server_requests)
{
	if (response) {
		...
		json_object_set(response_json,
				"requestId",
				json_object_get(server_request->request, "requestId"));
...
void ovndb_response(ovndb_t * ovndb, server_request_t * server_request,server_requests_t *server_requests)
{

	switch (server_request->type) {

	case REQ_RETRIEVE_REQUEST:
		process_retrieveRequest(ovndb, (retrieve_request_t *) server_request,server_requests);
		break;
	case REQ_NEW_NODE:
		process_newNodeRequest(ovndb, (new_node_request_t *) server_request,server_requests);
		break;
...
int main(int argc, char *argv[])
{
	...
	if (argc != 6) {
		printf
		    ("\nPlease provide the ip address and port for the server to connect\nAlso one contact point for the cassandra cluster and a contact point for the consensus server");
		exit(1);
	}
	...
	port = atoi(argv[5]);
	rc = zsocket_connect(router, "tcp://%s:%d",
				 argv[4], port);
	if (rc != 0) {
		printf
		    ("The ovn_server could't connect to %s:%d", argv[1], port);
	...
	ovndb_t *ovndb;
	ovndb_init(&ovndb, (const char **)&(argv[3]), 1, consensus_req_socket);
	server_requests_t server_requests;
